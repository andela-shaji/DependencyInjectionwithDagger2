package com.android.java.miss.dependencyinjectionwithdagger2;

import android.content.SharedPreferences;

import com.android.java.miss.dependencyinjectionwithdagger2.model.LineItem;
...
/**
 * This class file remembers the items that the users add the shopping cart in case there is a
 * configuration change aka phone call came in before they complete their shopping session
 * by checking out.
 */
...
public class ShoppingCart {
  ...
  private void initShoppingCart() {
    ...
    if (sharedPreferences.getBoolean(OPEN_CART_EXITS, false)) {
      String serializedCardItems = sharedPreferences.getString(SERIALIZED_CART_ITEMS, "");
      String serializedCustomer = sharedPreferences.getString(SERIALIZED_CUSTOMER, "");
      if (!serializedCardItems.equals("")) {
        shoppingCart = gson.<ArrayList<LineItem>>fromJson(serializedCardItems, new TypeToken<ArrayList<LineItem>>() {}.getType());
      }
    }
    updateApp();
  }
  ...
  public void updateItemQuery(LineItem item, int qty) {
